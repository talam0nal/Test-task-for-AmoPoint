...
class StaticPageController extends Controller
{
    ...
    public function AdminPublic(Request $oRequest, StaticPage $oModel)
    {
        ...
        if(!$oRequest->ajax())
            return redirect(route('admin_static_page_'))->with('success', $sMessage);
        else
            return json_encode(['status'=>'OK','message'=>$sMessage]);
    }

    public function AdminEditText(Request $oRequest, StaticPage $oModel)
    {
        $oModel->text = $oRequest->input('text','');
        $oModel->save();
        if(!$oRequest->ajax())
            return redirect(route('admin_static_page_'))->with('success', 'Success');
        else
    ...
    public function AdminDelete(StaticPage $oModel)
    {
        $oModel->delete();
        if(!request()->ajax())
            return redirect(route('admin_static_page_'))->with('success', 'Страница успешно удалена!');
        else
