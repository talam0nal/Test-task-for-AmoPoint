...
@section('content')
	<div class="card mb-4">
		...
		<div class="card-body">
			...
			@endif
			<form id="edit_form" method="post" class="form-horizontal" action="{{ route('admin_blog_category_store') }}" >
				{{ csrf_field() }}
				...
				@foreach($oModels as $key=>$oModel)
					<tr class="{{ $key % 2 == 0 ? 'odd gradeX' : 'even gradeC'}})">
						...
						<td class="center">
							...
							<a href="{{ route('admin_blog_category_public', ['oModel' => $oModel]) }}" class="btn btn-default btn-xs icon-btn md-btn-flat user-tooltip" title="" data-original-title="Ban"><i class="ion {{$oModel->published ? 'ion-md-eye-off' : 'ion-md-eye'}}"></i></a>
							...
							<form method="post" action="{{ route('admin_blog_category_delete', ['oModel'=>$oModel]) }}" style="display: none">
								{{ csrf_field() }}
								{{ method_field('DELETE') }}
							</form>
						</td>
	...
	<div class="modal fade" id="modal-edit">
		<div class="modal-dialog">
			<form method="post" class="modal-content" action="{{ route('admin_blog_category_store') }}" >
				{{ csrf_field() }}
